...
TEST_CASE("singular_value_decomposition") {
  ...
  auto sigma = make_matrix<double>(6_index, 5_index);
  auto diagonal_expression = [&](index_t i, index_t j) -> double{
    return i == j ? s(i) : 0;
  };
  executer(
    sigma = numeric_array::map_indexes(diagonal_expression, 6_index, 5_index)
  );
