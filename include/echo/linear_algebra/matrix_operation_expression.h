...
class MatrixOperationExpression
 ...
 public:
  ...
  MatrixOperationExpression(Pointer data, const Shape& shape)
      : htl::Pack<Shape>(shape), _data(data) {}
  auto data() { return _data; }
  auto data() const { return _data; }
  const auto* const_data() const { return _data; }
  const auto& shape() const { return htl::unpack<Shape>(*this); }
...
//------------------------------------------------------------------------------
template <class Structure, matrix_operation_t Operation, class Pointer,
          class Shape,
...
auto make_expression(numeric_array_expression_tag,
  ...
  return make_numeric_array_expression<
      ...
      get_dimensionality(expression), expression.evaluator());
}
}
}
